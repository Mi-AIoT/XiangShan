...
class LoadPipe(id: Int)(implicit p: Parameters) extends DCacheModule with HasPerfEvents with HasL1PrefetchSourceParameter {
  ...
  val s0_conflict_check_bank_oh = UIntToOH(addr_to_dcache_bank(io.lsu.req.bits.vaddr))
  val raw_wr_conflict_check = io.writehint.valid && (io.writehint.bits & s0_conflict_check_bank_oh).orR
