...
// the offset of the start pc within the cache line
  ...
  private val (s1_data, s1_maybeRvcMap) = (0 until MaxFetchReqNum).map { reqIdx =>
    val shamt    = s1_req(reqIdx).startVAddr(5, 1) // unit: half-word
    val range    = Fill(FetchBlockInstNum, 1.U(1.W)) >> (~s1_req(reqIdx).takenCfiOffset.bits).asUInt
    val dataMask = FillInterleaved(16, range)

    val rawData = VecInit((0 until DataBanks).map { bankIdx =>
      DataHoldBypass(
        Mux(
    ...
    val rawMaybeRvcMap = VecInit((0 until DataBanks).map { bankIdx =>
      DataHoldBypass(
        Mux(
          s1_bankMshrValid(reqIdx)(bankIdx),
    ...
    dontTouch(rawData)
    dontTouch(rawMaybeRvcMap)

    val data        = (Cat(rawData.asUInt, rawData.asUInt) >> (shamt << 4))(blockBits - 1, 0) & dataMask
    val maybeRvcMap = (Cat(rawMaybeRvcMap.asUInt, rawMaybeRvcMap.asUInt) >> shamt)(FetchBlockInstNum - 1, 0) & range
