...
//    storeQueue.io.enq.req(i).bits.lqIdx := loadQueue.io.enq.resp(i) // TODO: need it ?
  ...
  for (i <- 0 until io.enq.req.length) {
    val sqCanEnqueue          = io.enq.req(i).valid && io.enq.needAlloc(i)(1) && storeQueue.io.enq.canAccept
    val lqCanEnqueue          = io.enq.req(i).valid && io.enq.needAlloc(i)(0) && loadQueue.io.enq.canAccept

    XSError(sqCanEnqueue && !io.brqRedirect.valid && !loadQueue.io.enq.canAccept, s"must accept $i\n")
    XSError(lqCanEnqueue && !io.brqRedirect.valid && !storeQueue.io.enq.canAccept, s"must accept $i\n")
  }
...
class LsqEnqCtrl(implicit p: Parameters) extends XSModule
  ...
  for ((resp, i) <- io.fromRename.respEndPtr.zipWithIndex) { // end ptr calculate
    if(i == io.fromRename.respEndPtr.length - 1) {
      val lqOffset = loadFlowPopCount(i)
      resp.lqIdx := lqHeadPtr + lqOffset + loadQueueElem(i)
      val sqOffset = storeFlowPopCount(i)
  ...
  io.enqLsq.req.zip(io.fromDispatch.req).zip(io.fromDispatch.resp).zipWithIndex.foreach{ case (((toLsq, enq), resp), i) =>
    val do_enq = enq.valid && !io.redirect.valid && io.fromDispatch.canAccept && io.fromDispatch.iqAccept(i) &&
