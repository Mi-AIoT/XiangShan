...
abstract class PhysicalStoreQueueBase(implicit p: Parameters) extends LSQModule {
  ...
  private class ForwardModule(val param: ExeUnitParams)(implicit p: Parameters) extends LSQModule {
    ...
    for (i <- 0 until LoadPipelineWidth) {
      ...
      val s1MdpQueryResp = io.mdpQueryResp(i)
      val s1MdpQueryRespValid = s1MdpQueryResp.valid
      val s1MdpQueryRespBits = s1MdpQueryResp.bits
      ...
      val s1PreciseMdpWait     = s1MdpQueryRespValid && s1HasAddrInvalidVec(s1MdpQueryRespBits.value)
      ...
      s1AddrInvalidSqIdx     := s1MdpQueryRespBits
      ...
      val s2MdpQueryRespValid = RegEnable(s1MdpQueryRespValid, s1Valid)
      ...
      val s2PreciseMdpWait     = RegEnable(s1PreciseMdpWait, s1Valid)
...
class PhysicalStoreQueue(implicit p: Parameters) extends PhysicalStoreQueueBase with HasPerfEvents {
  ...
  for(i <- 0 until StoreQueuePhysicalSize) {
    ...
    XSError(ctrlEntries(i).addrValid && staSetValid &&
      (!selectBits.isLastRequest || !selectBits.cross4KPage), s"entry double allocate! index: ${i}\n")
...
//    XSError(ctrlEntries(i).dataValid && dataValidSet, s"[dataValid] double allocate! index: ${i}\n")
    XSError(!ctrlEntries(i).addrValid && deqCancel, s"double deq! index: ${i}\n")
  ...
  val addrReadyLookup = addrReadyLookupVec.map(ptr =>
      (ctrlEntries(ptr.value).addrValid || ctrlEntries(ptr.value).vecInactive) && ptr =/= physicalQueueUpper &&
        !redirectNeedClean // wait flag clean
  )
  val nextAddrReadyPtr = MuxCase(addrReadyPtr + PriorityEncoder(VecInit(addrReadyLookup.map(!_) :+ true.B)),
    Seq(
    (virtualStoreQueueRedirect.valid && addrReadyPtr.isAfter(virtualStoreQueueRedirect.bits)) -> virtualStoreQueueRedirect.bits
  ))
  ...
  val nextDataReadyPtr = MuxCase(dataReadyPtr + PriorityEncoder(VecInit(dataReadyLookup.map(!_) :+ true.B)),
    Seq(
        (virtualStoreQueueRedirect.valid && dataReadyPtr.isAfter(virtualStoreQueueRedirect.bits)) -> virtualStoreQueueRedirect.bits
  ))
