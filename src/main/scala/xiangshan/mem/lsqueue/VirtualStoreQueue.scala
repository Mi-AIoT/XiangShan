...
class VirtualStoreQueue[PhysicalQueuePtrType <: MultiFlagCircularQueuePtr[PhysicalQueuePtrType]] (
  ...
  */
  ...
  for(i <- 0 until LoadPipelineWidth) {
    ...
    val s1HitEntryIdx = PriorityEncoder(s1MdpHitVec.asUInt)
    val s1VirtualQueueSqIdx = WireDefault(0.U.asTypeOf(PhysicalQueuePtr.cloneType))

    when (s1VirtualQueueHit) {
      val hitEntryPtr = makeEntryPtrFromValue(s1HitEntryIdx)
      s1VirtualQueueSqIdx := getEntryStartSqIdx(hitEntryPtr)
    }

    io.toPhysicalQueue.mdpHitPtr(i).valid := s1VirtualQueueHit
    io.toPhysicalQueue.mdpHitPtr(i).bits  := s1VirtualQueueSqIdx
  }
  ...
  val enqPtrVecNext = enqPtrVec.map{ case ptr =>
    ...
    when(redirectReg.valid) {
      newPtr := ptr - PopCount(needCancel)
    }.otherwise {
  ...
  val redirectLastValidPtr = RegEnable(enqPtrVecNext.head - 1.U, redirectReg.valid)
  ...
  deqPtrVec.zipWithIndex.map{ case (ptr, i) =>
    if(i == 0) {
      ...
      retireVec(i) := isBefore(dataEntries(ptr.value).robIdx, robHeadPtr) &&
        ctrlEntries(ptr.value).allocated && retireVec(i - 1)
    }
  }

  val retireValid = retireVec.reduce(_ || _)
  val retireReqNum = deqPtrVec.zipWithIndex.map { case (ptr, i) =>
    Mux(retireVec(i), dataEntries(ptr.value).reqNum.asUInt, 0.U(log2Ceil(Size + 1).W))
  }.reduce(_ +& _)
  ...
  when(retireValid) {
    toPhysicalQueueRetirePtr := toPhysicalQueueRetirePtr + retireReqNum
  }
  io.toPhysicalQueue.retiredPtr := toPhysicalQueueRetirePtr

  XSError(
    retireValid && (toPhysicalQueueRetirePtr + retireReqNum) =/= getEntryEndSqIdx(deqPtrVecNext.head - 1.U),
  ...
  when(ctrlEntries(redirectLastValidPtr.value).allocated) {
    ...
    redirectRecoverPtr := toPhysicalQueueRetirePtr
  }
